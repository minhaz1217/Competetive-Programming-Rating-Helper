...
function getFullList(names, needCount, callback){
    ...
    names.forEach( function(name){
        getRecentRatings(name.trim(), needCount+5, function(contestList){
...
function contestAttended(names, ratingsFor, count, callback){
    var name, count,str=' {"result":[ ',indexCount=0;
    getRatedContest( ratingsFor, count, (ratedContests)=>{
        var contests = JSON.parse(ratedContests);
        getFullList(names, count, (fullList)=>{
            var nameList = JSON.parse(fullList);
            nameList.result.forEach(function(user){
                name = user.name;
                count = 0;
                for(var i=0;i<user.contest.length;i++){
                    for(var j=0;j<contests.result.length;j++){
                        if(contests.result[j].id == user.contest[i]){
                            count++;
                            break;
                        }
                    }
...
.post(function(req,res){
    ...
    var neededCount = Number(10);
    getFullList(names, neededCount, function(msg){
        res.send(JSON.parse(msg));
    });
})
;
//http://localhost:3000/api/cf/contestattended
router.route("/contestattended")
.get(function(req, res){
    ...
    ratingsFor.push("Div. 2");
    ratingsFor.push("Div. 3");
    var names = ['minhaz1217', 'anik_ewu'];
    var contestCount = 10;
    //res.send(contestCount);
    */
})
.post(function(req,res){
    ...
    contestAttended(names, ratingsFor, contestCount, (attended)=>{
       res.send(JSON.parse(attended));
    });
})
;
